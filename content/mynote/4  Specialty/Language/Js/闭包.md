...
function demo() {
	var arr = [];
	for (var i = 0; i < 10; i++)
		// 用 for 语句往数组 arr 中添加 10 个函数，该循环结束后变量 i 为 10。
		arr[i] = function () {
			// 虽然 for 语句的变量 i 一直自增到 10 才结束循环，但其内部的 10 个函数并没有执行，
			// 因此这个函数中的变量 i 虽然随着外部的变量 i 而变化，
			// 但并没有随着变量 i 的变化而执行 console.log。
			console.log(i);
		};
	return arr;
}
