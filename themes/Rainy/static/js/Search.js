...
function SearchBox(time) {
    ...
    var body = $('body');
    Mask.appendTo(body);
    Mask.attr('class', 'SearchDialogMask');
    // 创建对话框
    var Box = $('<dialog></dialog>');
    Box.appendTo(body);
    Box.attr('class', 'SearchDialogBox BoxShadow');
    // 创建搜索框内容
    var Content = $('<div></div>');
    ...
    $('.SearchDialogBox .CloseBox').click(function () {
        SearchDialog.fadeOut(time);
        $(".SearchDialogBox").remove();
        $(".SearchDialogMask").remove();
        SearchDialogMask.fadeOut(time);
    });
}
// 点击按钮弹出对话框
function GetEl() {
