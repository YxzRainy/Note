...
2. the otehr，表「两者中的另一个」时，后根名词单数；也可作代词来指代其他名词单数；表「其余的全部」时，后跟名词复数。
   - We have got tow cars, One is BMW，and the other is Audi.
3. the others 表特指「特定范围内的」其他的人或物。
4. others 相当于 other + 复数名词，表泛指；the others 相当于 the other + 复数名词，表特指。
   - I don't care what others think of me
   - I often have different ideas from the others in my office
5. another 只能用于三个或三个以上中的「另一个」；也用于「another + 数词 + 可数名词」，表「另外几...」；作限定词，表「再加一个」
...
2. any- 类复合不定代词，用于否定句或疑问句中；或用于 if 或 whether 之后。
