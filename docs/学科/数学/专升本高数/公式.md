...
除法移项 $\frac{a}{x} = b $，两边同 $\times \frac{x}{b} $ 得  $\frac{a}{b} = x$

 交叉相乘 $\frac{a}{c} = \frac{b}{d} $ 两端同时 $\times cd = ad = bc$
...
\\
sinx、cosx \qquad x \in R\\
\\
tanx =\frac{sinx}{cosx} \qquad x \in R \quad x \neq  \frac{\pi}{2} +k\pi \quad k \in Z\\
\\
cotx \qquad x \neq k\pi\\
\\
arctanx、arccotx \qquad x \in R、x \in (-\infty,+\infty)\\
\\
arcsinx、arccosx \qquad x \in [-1,1] \qquad \lvert x \rvert \leq 1
$$
...
若 $y = log_ax$，则 $x = a^y$；$y = lnx$，则 $ e^y = x$。
...
$$
sin(a \pm b) = sina ⋅ cosb \mp cosa ⋅ cosb\\
\\
